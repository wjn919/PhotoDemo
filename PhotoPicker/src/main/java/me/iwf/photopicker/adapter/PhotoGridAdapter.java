...
public class PhotoGridAdapter extends SelectableAdapter<PhotoGridAdapter.PhotoViewHolder> {
  ...
  private OnItemCheckListener onItemCheckListener    = null;//点击右上角方块
  private OnPhotoClickListener onPhotoClickListener  = null;//点击图片
  private View.OnClickListener onCameraClickListener = null;//点击相机调用

  public final static int ITEM_TYPE_CAMERA = 100;//展示相机
  public final static int ITEM_TYPE_PHOTO  = 101;//展示图片
  ...
  private void setColumnNumber(Context context, int columnNumber) {
    ...
    imageSize = widthPixels / columnNumber;//计算每张图片的大小
  }

  @Override public int getItemViewType(int position) {//获取类型，一种是展示相机，一种是不展示相机
  ...
  @Override public void onBindViewHolder(final PhotoViewHolder holder, int position) {
    ...
    if (getItemViewType(position) == ITEM_TYPE_PHOTO) {
      ...
      List<Photo> photos = getCurrentPhotos();//获取当前照片集合
      final Photo photo;
      ...
      boolean canLoadImage = AndroidLifecycleUtils.canLoadImage(holder.ivPhoto.getContext());//判断版本是否可以加载图片
      ...
      final boolean isChecked = isSelected(photo);//通过判断选中集合中是否有该photo的路径
