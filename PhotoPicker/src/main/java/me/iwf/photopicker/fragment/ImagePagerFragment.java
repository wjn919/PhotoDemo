...
import me.iwf.photopicker.EditorActivity;
...
public class ImagePagerFragment extends Fragment {
  ...
  public final static String SELECT_ARG_PATH = "SELECT_PATHS";
  ...
  public static ImagePagerFragment newInstance(List<String> selectedPath,List<String> paths, int currentItem) {

    ImagePagerFragment f = new ImagePagerFragment();

    Bundle args = new Bundle();

    args.putStringArray(SELECT_ARG_PATH, selectedPath.toArray(new String[selectedPath.size()]));
  ...
  public static ImagePagerFragment newInstance(List<String> selectedPath,List<String> paths, int currentItem, int[] screenLocation, int thumbnailWidth, int thumbnailHeight) {

    ImagePagerFragment f = newInstance(selectedPath,paths, currentItem);
  ...
  @Override public void onCreate(Bundle savedInstanceState) {
    ...
    if (bundle != null) {
      ...
      String[] selectPathArr = bundle.getStringArray(SELECT_ARG_PATH);
      paths.clear();
      selectPath.clear();
      if (pathArr != null) {
      ...
      if (selectPathArr != null) {
        ...
        selectPath = new ArrayList<>(Arrays.asList(selectPathArr));
      }
  ...
  @Nullable @Override public View onCreateView(LayoutInflater inflater, ViewGroup container,
    ...
    tvDone = (TextView)rootView.findViewById(R.id.tv_image_done);
    updapteDone();
    mViewPager = (ViewPager) rootView.findViewById(R.id.vp_photos);
    mViewPager.setAdapter(mPagerAdapter);
    mViewPager.setCurrentItem(currentItem);
    mViewPager.setOffscreenPageLimit(5);
    setTitle(getActivity(),currentItem);
    ...
    if (savedInstanceState == null && hasAnim) {
      ...
      getActivity().findViewById(R.id.v_selected).setOnClickListener(new View.OnClickListener() {
        @Override public void onClick(View view) {
          String path = paths.get(selectPosition);
          boolean isEnable = true;
          int selectedItemCount =  selectPath.size() + (selectPath.contains(path)? -1: 1);
          if (selectedItemCount > 9) {
            Toast.makeText(getActivity(), getString(R.string.__picker_over_max_count_tips, 9),
                    LENGTH_LONG).show();
            isEnable = false;
          }
          final Photo photo = new Photo(path.hashCode(),path);

          if (isEnable) {
            toggleSelection(photo);
            if(selectPath.contains(path)){
              getActivity().findViewById(R.id.v_selected).setSelected(true);
            }else{
              getActivity().findViewById(R.id.v_selected).setSelected(false);
            }
            updapteDone();
      ...
      getActivity().findViewById(R.id.tv_picker_return).setOnClickListener(new View.OnClickListener() {
        @Override
      ...
      tvDone.setOnClickListener(new View.OnClickListener() {
        ...
        public void onClick(View view) {
          ...
          intent.putStringArrayListExtra(PhotoPicker.KEY_SELECTED_PHOTOS,selectPath);
  ...
  public void onActivityResult(int requestCode, int resultCode, Intent data) {
    if(requestCode==PhotoPicker.REQUEST_CODE&&RESULT_OK==resultCode){
    ...
    }else if(resultCode==PhotoPicker.IS_EDITOR) {
      if(data!=null&&data.getBooleanExtra("ISEDITOR",false)){
        Intent intent = new Intent();
        intent.putExtra("ISEDITOR",true);
        getActivity().setResult(PhotoPicker.IS_EDITOR,intent);
        getActivity(). finish();
      }
    }else{//返回键返回回来的时候

      Intent intent = new Intent();
      intent.putStringArrayListExtra(PhotoPicker.KEY_SELECTED_PHOTOS,getSelectedPath());
      getActivity().setResult(RESULT_OK,intent);
      getActivity().finish();
  ...
  private void updapteDone() {


    tvDone.setEnabled(selectPath.size() > 0);
    ...
    if(selectPath.size()==0){
      tvDone.setText(R.string.__picker_continue);

    }else{
      tvDone.setText(getString(R.string.__picker_continue_with_count, selectPath.size(), 9));

    }
  }

  private void setTitle(final Activity activity, final int position) {
      final String path = paths.get(position);
    if(selectPath!=null&&selectPath.contains(path)){
      activity.findViewById(R.id.v_selected).setSelected(true);
    }else{
      activity.findViewById(R.id.v_selected).setSelected(false);
    }
  ...
  private void toggleSelection(Photo photo) {
    if (selectPath.contains(photo.getPath())) {
      selectPath.remove(photo.getPath());
    } else {
      selectPath.add(photo.getPath());
    }
  }
